#!/usr/bin/env python3
"""
Create a deployment using S3 storage - perfect for AWS ECS setup with Prefect 2.x!
"""
import asyncio
import boto3
...
async def create_s3_deployment():
    """Create a deployment using S3 storage - Prefect 2.x way"""
    ...
    try:
        ...
        # Create deployment using Prefect 2.x syntax
        deployment = await Deployment.build_from_flow(
            flow=s3_ecs_flow,
            name="s3-ecs-flow",
            work_queue_name="default",  # Use default work queue for process work pool
...
async def setup_s3_approach():
    ...
    print("📦 S3 + ECS Deployment Setup (Prefect 2.x)")
    print("=" * 50)
    ...
    print("✅ Works with Prefect 2.x")
    
    print("\n📋 What this will do:")
    print("1. Create S3 bucket: 'gellc-prefect-flows'")
    print("2. Upload flow files to S3")
    print("3. Create Prefect 2.x deployment pointing to S3")
